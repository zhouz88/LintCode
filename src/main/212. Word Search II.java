...
class Solution {
    ...
    private boolean dfs(int i, int j, TrieNode node, char[][] board, int m, int n, List<String> res) {
        if (!hasNext(i, j, node, board, m,n)) {
            return false;
        }
        if (node.nodes[board[i][j] - 'a'].word != null) {
            res.add(node.nodes[board[i][j] - 'a'].word);
            node.nodes[board[i][j] - 'a'].word = null;
        }
        char temp = board[i][j];
        board[i][j] = '*';
        for (int[] dir : DIRECTIONS) {
            int x = dir[0] + i;
            int y = dir[1] + j;
            if (dfs(x, y, node.nodes[temp - 'a'], board, m, n, res)) {
                return true;
            }
        }
        board[i][j] = temp;
    ...
    private boolean hasNext(int i, int j, TrieNode node, char[][] board, int m, int n) {
        return !(i<0||j<0||i>=m||j>=n||node==null||board[i][j]=='*'||node.nodes[board[i][j]-'a']==null);
    }
